...
def test_get(mock_get):
    ...
    mock_get.assert_called_with('https://api.rockarch.org/collections/1234', params={})

    mock_get.return_value.raise_for_status.side_effect = requests.exceptions.HTTPError()
    mock_get.return_value.text = mock_text
    assert client.get("collections/1234", params={"query": "yellow fever"}) == mock_text
    mock_get.assert_called_with('https://api.rockarch.org/collections/1234', params={"query": "yellow fever"})


@patch('requests.Session.get')
def test_get_paged(mock_get):
    collection_list = [{"title": "foo"}, {"title": "bar"}, {"title": "baz"}]
    mock_get.return_value.json.side_effect = [{"results": collection_list, "next": "collections"}, {"results": [], "next": None}]

    client = Client()
    assert list(client.get_paged("collections", params={"query": "yellow fever"})) == collection_list
